...
\subsection{Reasoning behavior: CPT makes the model concise and decisive}
The three axes above concern \emph{what} the model knows. We finally ask whether CPT changes
\emph{how} it reasons, independent of accuracy. On $200$ GSM8K problems, each sampled $k{=}5$
times under a shared 8-shot chain-of-thought scaffold, we measure chain length, self-correction
markers (``wait/actually/however/reconsider''), hedging markers, and self-consistency (agreement
of the final answer across the $5$ samples). Table~\ref{tab:cot} and Figure~\ref{fig:reasoning} show a striking behavioral
shift at the BioCPT stage: the reasoning chain \emph{shortens by 41\%} ($108.8\!\to\!64.4$
tokens) and self-correction \emph{nearly vanishes} ($0.370\!\to\!0.006$ backtracks per
...
low-redundancy sequence continuation --- appears to train a \emph{more direct, less
self-doubting} reasoning style. Subsequent SFT reverses this: chains grow past base length
($125.8$ tokens), backtracking and hedging return. This is the same CPT-shapes / SFT-re-shapes
signature seen on the knowledge axes, now visible in reasoning \emph{process} rather than
outcome --- a effect invisible to accuracy alone. (it and it-bio again coincide, confirming the
clean vocabulary ablation.)

\begin{figure}[h]\centering
\includegraphics[width=0.92\linewidth]{figs/fig3_reasoning.pdf}
\caption{Reasoning behavior on GSM8K. \textbf{Left:} biological CPT shortens the chain-of-thought
by 41\%. \textbf{Right:} self-correction (backtracks) nearly vanishes at BioCPT while accuracy is
preserved (even slightly improved); SFT reverses both. CPT instills a more concise, decisive
reasoning style.}
\label{fig:reasoning}
\end{figure}
...
8-shot CoT). CPT shortens and ``decisifies'' the chain without hurting accuracy; SFT reverses it.}
\label{tab:cot}
\begin{tabular}{lccccc}
\toprule
